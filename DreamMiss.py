...
class Tenc(nn.Module):
    ...
    def cacu_x(self, x):
        ...
        #      for c in range(len_states[r]-1):
        #         if random.random() < p_2:
        #             states[r,c:L-1]=states[r,c+1:L].clone()
        #             states[r,L-1]=self.item_num
        #             len_states[r]-=1
    ...
    def cacu_h(self, states, len_states,p,p1,p2):
        ...
        for r in range(B):
            ...
            if random.random() < p1*stability[r]*B:
             #sample item
             for c in range(len_states[r]-1):
                if random.random() < continuity[r,c]*p2*L:
                    #conduct omit
                    states[r,c:L-1]=states[r,c+1:L].clone()
                    states[r,L-1]=self.item_num
                    len_states[r]-=1
...
if __name__ == '__main__':
    ...
    best_epoch = 0
    missing={}
    entropy_h=torch.ones(args.batch_size)
    prob_x=torch.ones(args.batch_size,seq_size)
